...
class MultilayerAnalysis:
    ...
    def highlightComponents(self,scoredDF,mask=None,output=None,pretty=None):
        
        print("Starting highlightComponents()...")
       
        numRows = scoredDF.count()
        if not mask:
        ...
        return winner.split("_")[1], scoredDF[["layer", min(fitKey, key=fitKey.get)]]


   

    def communityPlot(self,scoredDF,rank,factor,output=False,display=False,verbose=False):
        ...
        fig.suptitle('Personality Level as a Function of Factor {} Tensor Component'.format(factor))
        ...
        # plt.tight_layout()
        if output:
            fpath = os.path.join(output,"personality_activity-factor_{}-rank{}.png".format(factor,rank))
            plt.savefig(fpath,dpi=600)
        if display:    
            plt.show()

    def detectCommunities(self,commDF,ffmsDF,output=None,plot=False,display=False,verbose=False):
        ...
        if plot:
            self.communityPlot(saDF,rank,"A",output=output,display=display)
            self.communityPlot(sbDF,rank,"B",output=output,display=display)
    ...
    def run(self):
        ...
        aScored,bScored = self.detectCommunities(commDF=commDF,ffmsDF=ffmsDF,plot=True,display=True,output=self.csv_path)

        print("The best scored component of A is: \n{}".format(aScored))

        # profile = [("OPN",1),("CSN",0),("EXT",1),("AGR",0),("NEU",-1)]
        profile = [("OPN",1),("CSN",1),("EXT",0),("AGR",0),("NEU",-1)]
        ...
        bestA = "A_"+compA
        bestB = "B_"+compB
        print("The best component of A is: \n{}".format(compA))
        print("The best component of B is: \n{}".format(compB))

        numNodes = 5
        topA = self.getTopN(bestA,commDF,N=numNodes)#,values=True) # return numpy.ndarray
        topB = self.getTopN(bestB,commDF,N=numNodes)#,values=True) # return numpy.ndarray

        # print("topA:\n{}".format(topA))
        # print("topB:\n{}".format(topB))

        bestADF = self.getNodes(nodeDF,topA)
        bestBDF = self.getNodes(nodeDF,topB)


        print("Our profile:")
        print(profile)
        print("The best A folks")
        print(bestADF.head(numNodes))
        print("The best B folks")
        print(bestBDF.head(numNodes))
